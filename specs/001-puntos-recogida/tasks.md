...
- [ ] T142 CRITICAL: Ejecutar los fixtures de `extensions/ocultar-envios/tests/fixtures/*.json` y `extensions/renombrar-recogida/tests/fixtures/*.json` sobre el wasm compilado (`shopify app function run --input <fixture> --export run`) desde `npm test`, comparando `output` y fallando si el fixture de peor caso supera el 50 % del límite de instrucciones; omitir (como los contract tests de Firestore sin emulador) cuando no exista `dist/function.wasm` — requiere T001 + `npm run typegen`/`npm run build` en cada Function (bloqueado en la nube, igual que T090) per Constitution VII, §18.4, FR-091 (missing)
- [x] T143 Agregar un test Vitest por Function (`extensions/ocultar-envios/src/fixtures.test.ts` y `extensions/renombrar-recogida/src/fixtures.test.ts`) que recorra `tests/fixtures/*.json`, pase cada `input` por el `run` de `src/index.ts` y verifique que el resultado es igual al `output` del fixture, para que `npm test` valide los 11 fixtures sin wasm per FR-091, §18.4 (partial)
- [x] T144 Configurar umbrales de cobertura de líneas ≥90 % (`coverage.thresholds`) en los `vitest.config.ts` de `app`, `packages/contratos`, `packages/selector-carrito`, `extensions/ocultar-envios`, `extensions/renombrar-recogida` y `deploy`, limitados a los scopes de dominio/aplicación del §24.1, para que `npm run test:coverage` falle por debajo del umbral per FR-092, §21.4 (partial)
- [x] T145 Restaurar la ruta de inicio de sesión del template `app/routes/auth.login/route.tsx` (+ `error.server.tsx`), con textos en español, para que `/auth/login` (destino de `authenticate.admin`/`login` sin `shop`) no responda 404 per §15.1 (missing)
- [x] T146 Quitar la página de ejemplo del template `app/routes/app.additional.tsx` y reemplazar la navegación en inglés de `app/routes/app.tsx` ("Home"/"Additional page") por textos de `app/i18n/es.ts` per FR-055, NFR-16 (unrequested)
- [x] T147 Reescribir la landing pública `app/routes/_index/route.tsx` (hoy con el placeholder en inglés "A short heading about [your app]") en español, describiendo la app y el formulario de dominio de la tienda per NFR-16, FR-035, SC-012 (contradicts)
- [x] T148 Alinear los logs del backend con el catálogo del §22: emitir `proxy.puntos.respuesta` (tienda, estado de caché fresca/vencida/cargada/stale, total, duracionMs) en `app/routes/proxy.puntos.tsx` y `puntos.carga` (páginas, total, inválidos, duplicados, costo, duracionMs) en `ListarPuntosRecogida`/`FuentePuntosShopify`; renombrar `puntos.carga_fallida`→`puntos.carga.error`, `puntos.invalido_descartado`→`puntos.invalido`, `puntos.identificador_duplicado`→`puntos.duplicado` (con `gids`) y `personalizaciones.activadas`→`personalizaciones.activacion`; registrar `webhook.recibido` también en `app/uninstalled` y `app/scopes_update`; agregar `requestId` desde `traceparent`/`x-cloud-trace-context`; incluir el stack en el log de errores 500 (sin secretos), actualizando README §18 per §22, NFR-14, §15.6 (partial)
- [x] T149 Marcar `stale: true` solo cuando se sirve una copia vencida porque Shopify falló (stale-if-error), no en el camino normal de stale-while-revalidate, en `app/application/use-cases/listarPuntosRecogida.ts` y sus tests per CT-03, FR-016 (contradicts)
- [x] T150 Implementar el respaldo del EC-15 en `packages/selector-carrito/src/infraestructura/InsertadorWidgets.ts`: si ningún botón coincide con `selector_botones_checkout` (y no hay `selector_insercion`), mostrar el widget en la posición del contenedor `.pr-embed-raiz` del embed; test jsdom incluido y vigilando el límite de 20 KB del bundle (hoy 19 717 bytes) per EC-15, NFR-11 (missing)
- [x] T151 Agregar modo watch a `packages/selector-carrito/build.mjs` (`--watch`) y hacer que `npm run dev` lo ejecute junto a `shopify app dev --config dev` per §21.4, §21.2 (partial)
- [x] T152 Construir el `href` del enlace compacto de `InsertadorWidgets.ts` con `Shopify.routes.root` (o `rutaRaiz` de `#pr-config`) en lugar de `/cart` fijo, para tiendas con prefijo de idioma per EC-17, EC-24 (partial)
- [x] T153 Garantizar el apagado ordenado del contenedor ante `SIGTERM` (arrancar el servidor sin `npm` como PID 1, p. ej. `CMD ["node_modules/.bin/react-router-serve", "./build/server/index.js"]` o `docker run --init`/tini) y documentarlo en el README per §20.2 (partial)
- [x] T154 Leer `apiKey` en `app/routes/app.tsx` desde la configuración validada del contenedor (`contenedor.config`) en lugar de `process.env.SHOPIFY_API_KEY` directo per Constitution V, §15.5 (contradicts)

---

## Phase 13: Convergence

- [ ] T155 Hacer que la etapa `build` del `Dockerfile` construya solo el backend: agregar el script `build:backend` (`react-router build`) en `package.json` (y dejar `build` = `npm run build:backend && npm run build:embed`), ejecutar `npm run build:backend` en el `Dockerfile` en lugar de `npm run build` —hoy falla con `Cannot find module …/packages/selector-carrito/build.mjs`, porque ni `packages/selector-carrito` ni `extensions/` (excluida en `.dockerignore`) están en el contexto—, y validarlo con `docker build` (si `dockerd` arranca en la sesión) o reproduciendo en un directorio temporal los mismos `COPY` + `npm ci` + build; actualizar README §14 per FR-075, §20.2, US-6/AC1 (contradicts)
- [ ] T156 Hacer que `npm run typecheck` ejecute `tsc --noEmit` en todos los workspaces (p. ej. `react-router typegen && tsc --noEmit && npm run typecheck --workspaces --if-present`) y corregir los 2 errores de `packages/selector-carrito/src/main.ts` sin cambiar el comportamiento: `interruptorDemo` con `exactOptionalPropertyTypes` (omitir la propiedad en vez de pasar `undefined`) y el tipo de retorno de `atributosModoDemo` (`Record<string, string>`, que es lo que espera `ClienteCarrito.actualizarAtributos`); actualizar README §12 (descripción de `typecheck` y el estado de tests/cobertura, hoy desactualizado en 240 tests) per §21.4, Constitution II (partial)
- [ ] T157 Dejar en la imagen final solo dependencias de ejecución: mover a `devDependencies` los paquetes que solo usan el build o el desarrollo (`@shopify/cli`, `@react-router/dev`, `@react-router/fs-routes`, `vite-tsconfig-paths`) regenerando `package-lock.json` con `npm install`; comprobar que `npm run build`, `npm test`, `npm run dev`/`shopify` locales siguen funcionando y que el servidor arranca con `npm ci --omit=dev` + `react-router-serve` (`GET /healthz` → 200). Hoy `npm ci --omit=dev` instala 293 MB (typescript, vite, prettier, `@ast-grep`, esbuild con un aviso de `npm audit`) y sin esos paquetes 139 MB con 0 vulnerabilidades; medir el tamaño de la imagen frente al objetivo de ≤ 250 MB y documentarlo en README §14 y §19 per §20.2, §23 (partial)
