...
- [ ] T143 Agregar un test Vitest por Function (`extensions/ocultar-envios/src/fixtures.test.ts` y `extensions/renombrar-recogida/src/fixtures.test.ts`) que recorra `tests/fixtures/*.json`, pase cada `input` por el `run` de `src/index.ts` y verifique que el resultado es igual al `output` del fixture, para que `npm test` valide los 11 fixtures sin wasm per FR-091, §18.4 (partial)
- [ ] T144 Configurar umbrales de cobertura de líneas ≥90 % (`coverage.thresholds`) en los `vitest.config.ts` de `app`, `packages/contratos`, `packages/selector-carrito`, `extensions/ocultar-envios`, `extensions/renombrar-recogida` y `deploy`, limitados a los scopes de dominio/aplicación del §24.1, para que `npm run test:coverage` falle por debajo del umbral per FR-092, §21.4 (partial)
- [ ] T145 Restaurar la ruta de inicio de sesión del template `app/routes/auth.login/route.tsx` (+ `error.server.tsx`), con textos en español, para que `/auth/login` (destino de `authenticate.admin`/`login` sin `shop`) no responda 404 per §15.1 (missing)
- [ ] T146 Quitar la página de ejemplo del template `app/routes/app.additional.tsx` y reemplazar la navegación en inglés de `app/routes/app.tsx` ("Home"/"Additional page") por textos de `app/i18n/es.ts` per FR-055, NFR-16 (unrequested)
- [ ] T147 Reescribir la landing pública `app/routes/_index/route.tsx` (hoy con el placeholder en inglés "A short heading about [your app]") en español, describiendo la app y el formulario de dominio de la tienda per NFR-16, FR-035, SC-012 (contradicts)
- [ ] T148 Alinear los logs del backend con el catálogo del §22: emitir `proxy.puntos.respuesta` (tienda, estado de caché fresca/vencida/cargada/stale, total, duracionMs) en `app/routes/proxy.puntos.tsx` y `puntos.carga` (páginas, total, inválidos, duplicados, costo, duracionMs) en `ListarPuntosRecogida`/`FuentePuntosShopify`; renombrar `puntos.carga_fallida`→`puntos.carga.error`, `puntos.invalido_descartado`→`puntos.invalido`, `puntos.identificador_duplicado`→`puntos.duplicado` (con `gids`) y `personalizaciones.activadas`→`personalizaciones.activacion`; registrar `webhook.recibido` también en `app/uninstalled` y `app/scopes_update`; agregar `requestId` desde `traceparent`/`x-cloud-trace-context`; incluir el stack en el log de errores 500 (sin secretos), actualizando README §18 per §22, NFR-14, §15.6 (partial)
- [ ] T149 Marcar `stale: true` solo cuando se sirve una copia vencida porque Shopify falló (stale-if-error), no en el camino normal de stale-while-revalidate, en `app/application/use-cases/listarPuntosRecogida.ts` y sus tests per CT-03, FR-016 (contradicts)
...
- [ ] T151 Agregar modo watch a `packages/selector-carrito/build.mjs` (`--watch`) y hacer que `npm run dev` lo ejecute junto a `shopify app dev --config dev` per §21.4, §21.2 (partial)
- [ ] T152 Construir el `href` del enlace compacto de `InsertadorWidgets.ts` con `Shopify.routes.root` (o `rutaRaiz` de `#pr-config`) en lugar de `/cart` fijo, para tiendas con prefijo de idioma per EC-17, EC-24 (partial)
- [ ] T153 Garantizar el apagado ordenado del contenedor ante `SIGTERM` (arrancar el servidor sin `npm` como PID 1, p. ej. `CMD ["node_modules/.bin/react-router-serve", "./build/server/index.js"]` o `docker run --init`/tini) y documentarlo en el README per §20.2 (partial)
- [ ] T154 Leer `apiKey` en `app/routes/app.tsx` desde la configuración validada del contenedor (`contenedor.config`) en lugar de `process.env.SHOPIFY_API_KEY` directo per Constitution V, §15.5 (contradicts)
